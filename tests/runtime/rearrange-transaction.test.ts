...
import { ref, onScopeDispose, watch } from '../../packages/arrange-vue-reactivity/src/index.ts'
import { defineArrangable } from '../../packages/arrange-vue-runtime-core/src/apiDefineArrangable.ts'
import { Composition } from '../../packages/arrange-vue-runtime-core/src/rearrange.ts'
import { nextTick } from '../../packages/arrange-vue-runtime-core/src/scheduler.ts'
import { onMounted, onUnmounted, onUpdated } from '../../packages/arrange-vue-runtime-core/src/apiLifecycle.ts'
import type { CompositionHost } from '../../packages/arrange-vue-runtime-core/src/rearrangeNode.ts'
import type { ArrangableDefinition } from '../../packages/arrange-vue-runtime-core/src/arrangable.ts'
...
function controlledHost() {
    ...
    return {
        create(definition: ArrangableDefinition) { return new Composition({ host, config: { errorHandler(error) { errors.push(error) } }, provides: {}, definitions: {} }, definition, {}) },
        finish(error?: Error) {
...
test('AB 到 AC 等待 apply 成功后才释放 B，准备阶段不发送成功通知', async () => {
    ...
    const B = defineArrangable({ setup() {
        onScopeDispose(() => events.push('释放 B'))
        return () => {}
    } })
    const C = defineArrangable({ setup() {
        events.push('创建 C')
        onMounted(() => events.push('挂载 C'))
        return () => {}
    } })
    const App = defineArrangable({ setup(_props, { call }) { return () => {
        call(0, A, {})
        call(1, selected.value ? C : B, {})
    } } })
    const native = controlledHost()
    const composition = native.create(App)
    composition.mount()
    native.finish()
...
test('纯值 apply 失败恢复旧参数和求值缓存，再次变化能够正确提交', async () => {
    ...
    const Child = defineArrangable({ props: { value: { type: Number, required: true } }, setup(values) { props = values; return () => {} } })
...
test('候选嵌套调用回滚不发送任何成功卸载通知，深层退出只清理一次', async () => {
    ...
    const events: string[] = []
    const Leaf = defineArrangable({ setup() {
...
test('结构代码自行捕获创建异常也不能把失败候选提交成空输出', async () => {
    ...
    const App = defineArrangable({ setup: (_props, { call }) => () => {
...
test('同一事务内二次失效复用候选实例，并只发送一次成功通知', () => {
    ...
    let mounts = 0
    let evaluations = 0
    const Child = defineArrangable({ setup() {
        setups++
        onMounted(() => mounts++)
        return () => {}
    } })
...
test('成功回调抛错不撤销已提交状态，后续通知和更新继续执行', async () => {
    ...
    const App = defineArrangable({ setup() {
        onMounted(() => { events.push('挂载'); throw new Error('业务挂载通知失败') })
...
test('候选清理抛错仍释放其余资源，保留旧生命且允许重试', async () => {
    ...
    const revision = ref(0)
    const events: string[] = []
    const B = defineArrangable({ setup() { onScopeDispose(() => events.push('释放 B')); return () => {} } })
    const C = defineArrangable({ setup() {
        onScopeDispose(() => { events.push('清理一'); throw new Error('业务清理失败') })
...
test('开启事务失败能清理运行状态，之后可以重新挂载', () => {
    ...
    const App = defineArrangable({ setup: () => () => {} })
    const composition = new Composition({ host, config: {}, provides: {}, definitions: {} }, App, {})
    assert.throws(() => composition.mount(), /开启事务失败/)
    assert.equal(rollbacks, 1)
    fail = false
    composition.mount()
    assert.equal(composition.root.entries.length, 1)
    composition.dispose()
})
