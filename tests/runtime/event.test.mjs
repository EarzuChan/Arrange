...
import { Box, m, dp, rememberFocusRequester, rememberInteractionState, useFocusManager } from "../../packages/runtime/src/index.mjs";
...
test("clickable is triggered through Arrange test rule", async () => {
  let clicks = 0;
  const ui = await renderArrange(() => h(Box, { modifier: m.size(dp(40), dp(20)).clickable(() => { clicks += 1; }).testTag("button") }));
  ...
  const ui = await renderArrange(() => h(Box, {}, [
    h(Box, {
      modifier: m
        ...
        .onFocusChanged((state) => changes.push(state.focused))
        .testTag("field"),
    }),
...
test("clickable is focusable by default and exposes focus through InteractionState", async () => {
  ...
  const ui = await renderArrange(() => h(Box, {
    modifier: m
      .size(dp(40), dp(20))
      .clickable({ interactionState: interaction, onClick: () => { clicks += 1; } })
      .testTag("button"),
  }));
