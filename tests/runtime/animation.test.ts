...
import {tween, linearEasing, animatedColorAsRef, animatedDpAsRef, animatedNumberAsRef, createManualAnimationClock, ref, createTransition} from "../../packages/runtime/src/index.ts"
...
test("all value groups animate on one clock and reject changing vector dimensions", async () => {
    const {animatedNumberArrayAsRef, animatedOffsetAsRef, animatedSizeAsRef, animatedRectAsRef} = await import('../../packages/runtime/src/index.ts')
    const clock = createManualAnimationClock()
    ...
    vector.value = [10, 20]; offset.value = {x: 10, y: 20}; size.value = {width: 30, height: 40}; rect.value = {x: 10, y: 20, width: 30, height: 40}
...
test("spring retarget preserves velocity and is independent of frame subdivision", async () => {
    const {spring} = await import('../../packages/runtime/src/index.ts')
...
test("Transition publishes coherent channels even to synchronous watchers and retires children", async () => {
    const {watch, animationStats} = await import('../../packages/runtime/src/index.ts')
...
test("scope disposal cancels queued frames and target subscriptions; snap and delayed tween settle", async () => {
    const {effectScope, snap, animationStats} = await import('../../packages/runtime/src/index.ts')
    const baseline = animationStats.activeAnimations
    ...
    target.value = 100
    clock.advanceBy(10); assert.equal(animated.value, 0)
    clock.advanceBy(60); assert.equal(animated.value, 50)
    scope.stop()
    target.value = 200; clock.advanceBy(500)
...
test("completion callbacks observe the complete frame and stopping a sibling suppresses its queued callback", () => {
    ...
    const x = animatedNumberAsRef(xTarget, {clock, animationSpec: spec, finished() { observed.push([x.value, y.value]); y.stop() }})
    const y = animatedNumberAsRef(yTarget, {clock, animationSpec: spec, finished() { throw new Error('retired completion ran') }})
    xTarget.value = 10; yTarget.value = 20
    clock.advanceBy(100)
...
test("group animation tracks reactive fields and bounds size overshoot", async () => {
    const {animatedOffsetAsRef, animatedSizeAsRef, spring} = await import('../../packages/runtime/src/index.ts')
...
test("Transition child mappings follow their actual reactive reads and stop unsubscribes them", () => {
    ...
    target.value = true; clock.advanceBy(100)
    assert.equal(width.value, 100)
    extent.value = 200; clock.advanceBy(50)
    assert.equal(width.value, 150)
    transition.stop()
    extent.value = 300; clock.advanceBy(100)
    assert.equal(width.value, 150)
    assert.equal(clock.pendingFrames, 0)
})
