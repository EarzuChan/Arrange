...
import {Column, Text, Box, m, dp, Color, encodeBridgeBatch} from "../../../packages/runtime/src/index.ts"
import {h, renderToBridgeOps} from "../../../packages/runtime/src/test/index.ts"
...
const ops = renderToBridgeOps(() => h(Column, {modifier: m.padding(dp(8)).testTag("root")}, [
    ...
    h(Box, {
        modifier: m.size(dp(16), dp(8)).background(Color(0xFF112233)).clickable(() => {
        }).testTag("box")
    }),
]))
