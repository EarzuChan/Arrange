...
export class Modifier {
    ...
    widthIn(args: { min?: number; max?: number }): Modifier { return this.#add("widthIn", { ...args }) }
    heightIn(args: { min?: number; max?: number }): Modifier { return this.#add("heightIn", { ...args }) }
    sizeIn(args: SizeRange): Modifier { return this.#add("sizeIn", { ...args }) }
    defaultMinSize(args: Pick<SizeRange, "minWidth" | "minHeight">): Modifier { return this.#add("defaultMinSize", { ...args }) }
    ...
    graphicsLayer(args: GraphicsLayerOptions = {}): Modifier { return this.#add("graphicsLayer", { ...args }) }
    ...
    hoverable(args: EnabledOptions = {}): Modifier { return this.#add("hoverable", { enabled: true, ...args }) }
    focusable(arg: boolean | EnabledOptions = true): Modifier { return this.#add("focusable", typeof arg === "boolean" ? { enabled: arg } : { enabled: true, ...arg }) }
    // pointerInput 将来会在 native typed pointer event slot、派发、释放与测试齐全后再正规添加回来；当前故意不公开半支持 API
    verticalScroll(state: ScrollStateLike, args: EnabledOptions = {}): Modifier { return this.#add("verticalScroll", { state: scrollStateSnapshot(state), enabled: true, ...args }) }
    horizontalScroll(state: ScrollStateLike, args: EnabledOptions = {}): Modifier { return this.#add("horizontalScroll", { state: scrollStateSnapshot(state), enabled: true, ...args }) }
