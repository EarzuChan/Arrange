...
const validateNativeTemplate: NodeTransform = (node, context) => {
    ...
    const fail = (message: string, loc = node.loc) => {context.onError(Object.assign(new SyntaxError(message), { code: 'ARRANGE_TEMPLATE', loc }))}

    const host = isHostTag(node.tag) ? node.tag : null

    for (const prop of node.props) {
        const name = prop.type === NodeTypes.ATTRIBUTE ? prop.name : prop.name === 'bind' && prop.arg?.type === NodeTypes.SIMPLE_EXPRESSION && prop.arg.isStatic ? prop.arg.content : null
