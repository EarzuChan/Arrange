import { computed, effectScope, pauseTracking, ReactiveEffect, resetTracking, shallowRef, type EffectScope } from '@arrange/vue-reactivity'
import { arrangeParameterName } from '@arrange/vue-shared'
...
export class RearrangeScope {
    ...
    constructor(readonly composition: Composition, readonly owner: ArrangableInstance | null, readonly parent: RearrangeScope | null, private program: StructureProgram, readonly source: string | undefined = owner?.source ?? parent?.source) {
        this.effect = this.makeEffect()
        this.job = () => {
            if (this.retired || this.paused || this.owner?.isUnmounted || !this.effect.dirty) return
            composition.schedule(this)
        }
    ...
    private makeEffect(): ReactiveEffect {
        ...
        effect.scheduler = () => {
            if (this.retired || this.composition.reverting) return
            if (this.composition.preparing) this.composition.enqueue(this)
    ...
    refresh(program: StructureProgram): void {
        ...
        this.composition.onRollback(() => { this.program = previous })
        this.program = program
        this.composition.enqueue(this, program !== previous)
    }
    ...
    private execute(): void {
        ...
        this.index.clear()
        for (const entry of this.composition.entries(this)) {
            let keys = this.index.get(entry.position)
            if (!keys) this.index.set(entry.position, keys = new Map())
            keys.set(entry.key, entry)
        }
        this.composition.preserve(this.index, () => () => this.publish(this.entries))
        this.used.clear()
        ...
        try {
            ...
            this.composition.stage(this, this.next)
            this.composition.preserve(this.parameterObjects, () => {
                this.composition.onCommit(() => {
    ...
    resume(): void {
        visitScopes(this, scope => {
            ...
            this.composition.enqueue(scope, true)
        })
    }
...
export class Composition {
    ...
    private isActive(scope: RearrangeScope): boolean {
        ...
        let active = true
        while (current) {
            if (this.activeScopes.has(current)) { active = this.activeScopes.get(current)!; break }
            pending.push(current)
            if (current.retired || current.paused) { active = false; break }
            const parent: RearrangeScope | null = current.parent
            if (parent && !this.entries(parent).some(entry => (entry.kind === 'call' ? entry.instance.structure : entry.scope) === current)) { active = false; break }
            current = parent
        }
    ...
    runValue(update: ValueTask): void {
        ...
        if (this.applying) { this.deferredValues.add(update); return }
...
export function callArrangable<D extends ArrangableDefinition>(position: CallPosition, definition: D, inputs: PropInputs<ArrangableProps<D>>, contents: Contents = {}, metadata: CallMetadata = {}): void {
    ...
    try {
        ...
        if (previous?.kind === 'call' && previous.instance.type === definition) {
            ...
            if (updateContents(entry.instance, contents)) scope.composition.enqueue(entry.instance.structure, true)
        } else {
            const instance = new ArrangableInstance(definition, scope.owner, scope.composition.context, metadata.source)
...
function initializeInstance(instance: ArrangableInstance, parent: RearrangeScope, inputs: PropInputs<Data>, contents: Contents, metadata: CallMetadata): void {
    instance.composition = parent.composition
    ...
    try {
        ...
        instance.structure = new RearrangeScope(parent.composition, instance, parent, () => {
            ...
            parent.composition.notify(instance, instance.isMounted ? LifecycleHooks.UPDATED : LifecycleHooks.MOUNTED)
        }, metadata.source)
        parent.composition.enqueue(instance.structure, true)
    } catch (error) {
...
function updateContents(instance: ArrangableInstance, contents: Contents): boolean {
    ...
    instance.composition.onRollback(() => { instance.contents = before; version.value = previousVersion })
    instance.contents = contents
...
export function arrangeScope(position: CallPosition, program: StructureProgram, key?: RearrangeKey, source?: string): void {
    ...
    if (previous?.kind === 'scope') {
        ...
        const scope = new RearrangeScope(parent.composition, parent.owner, parent, program, source)
...
export function retainContent(position: CallPosition, key: RearrangeKey, content: Content, max = Infinity): void {
    ...
    if (!cache) {
        ...
        parent.composition.onRollback(() => { parent.retainedContents.delete(position) })
    }
    const retained = cache
    const candidate = parent.composition.entries(parent).find(entry => entry.kind === 'scope' && entry.retention === retained && entry.position === position && Object.is(entry.key, key))
    let entry = candidate?.kind === 'scope' ? candidate : retained.entries.get(key)
    if (entry) {
        entry.scope.refresh(content)
        if (entry.scope.paused) {
            entry.scope.resume()
            parent.composition.onRollback(() => entry!.scope.pause())
            visitScopes(entry.scope, scope => {
                if (scope.owner?.structure === scope) parent.composition.notify(scope.owner, LifecycleHooks.ACTIVATED)
            })
        }
    } else {
        const scope = new RearrangeScope(parent.composition, parent.owner, parent, content)
    ...
    parent.composition.onCommit(() => {
        if (retained.closed || selected.scope.retired || !parent.composition.entries(parent).includes(selected)) return
...
function collectNodes(scope: RearrangeScope): RearrangeNode[] {
    ...
    const pending = [...scope.composition.entries(scope)].reverse()
    while (pending.length) {
        ...
        if (entry.kind === 'call' && entry.instance.node) nodes.push(entry.instance.node)
            ...
            const entries = scope.composition.entries(nested)
...
function retireEntries(entries: readonly Entry[], notify = true): void {
    ...
    while (pending.length) {
        const next = pending.pop()!
        if (typeof next === 'function') { safely(next); continue }
...
class ObjectParameterBinding {
    ...
    constructor(read: () => Record<string, unknown>, readonly scope: RearrangeScope) {
        this.binding = scope.lifetime.run(() => new ValueBinding(read, scope.owner!, value => {
            ...
            scope.composition.preserve(this, () => {
