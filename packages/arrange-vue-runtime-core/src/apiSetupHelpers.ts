...
type PropsWithDefaults<T, Defaults extends InferDefaults<T>> = Readonly<Omit<T, keyof Defaults>> & {
    readonly [K in keyof Defaults as K extends keyof T ? K : never]-?: K extends keyof T ? Defaults[K] extends undefined ? T[K] : NotUndefined<T[K]> : never
}
 ...
 * @internal
 */
export function normalizePropDeclarations(
    props: ArrangablePropsOptions,
): ArrangableObjectPropsOptions {
