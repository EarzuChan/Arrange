import { BindingTypes, TS_NODE_TYPES, extractIdentifiers, isFunctionType, isInDestructureAssignment, isReferencedIdentifier, isStaticProperty, unwrapTSNode, walkFunctionParams } from '../../template/index.ts'
import { genPropsAccessExp } from '@arrange/shared'
import type { BlockStatement, Expression, Identifier, Node, ObjectPattern, Program, VariableDeclaration } from '@babel/types'
...
export function processPropsDestructure(ctx: ScriptCompileContext, declId: ObjectPattern): void {
    ...
    const registerBinding = (key: string, local: string, defaultValue?: Expression) => {
...
export function transformDestructuredProps(ctx: ScriptCompileContext, arrangeImportAliases: Record<string, string>): void {
    ...
    function walkScope(node: Program | BlockStatement, isRoot = false) {
        for (const stmt of node.body) {
            if (stmt.type === 'VariableDeclaration') walkVariableDeclaration(stmt, isRoot)
    ...
    function rewriteId(id: Identifier, parent: Node, parentStack: Node[]) {
        if (
            (parent.type === 'AssignmentExpression' && id === parent.left) || parent.type === 'UpdateExpression'
        ) {
    ...
    walk(ast, {
        enter(node: Node, parent: Node | null) {
            ...
            if (node.type === 'Identifier') {
                if (
                    isReferencedIdentifier(node, parent!, parentStack) && !excludedIds.has(node)
                ) {
        ...
        leave(node: Node, parent: Node | null) {
            ...
            if (
                (node.type === 'BlockStatement' && !isFunctionType(parent!)) || isFunctionType(node) || node.type === 'CatchClause' || node.type === 'ForOfStatement' || node.type === 'ForInStatement' || node.type === 'ForStatement'
            ) {
                popScope()
            }
        },
