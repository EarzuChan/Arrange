...
export default class Tokenizer {
    ...
    private stateBeforeAttrName(c: number): void {
        if (c === CharCodes.Gt) {
            ...
            if ((__DEV__ || !false) && this.peek() !== CharCodes.Gt) {
            ...
            if ((__DEV__ || !false) && c === CharCodes.Eq) {
    ...
    private handleAttrStart(c: number) {
        if (c === CharCodes.LowerA && this.peek() === CharCodes.Dash) {
    ...
    private stateInAttrName(c: number): void {
        if (c === CharCodes.Eq || isEndOfTagSection(c)) {
            ...
            (__DEV__ || !false) && (c === CharCodes.DoubleQuote || c === CharCodes.SingleQuote || c === CharCodes.Lt)
        ) {
    ...
    private stateInAttrValueNoQuotes(c: number): void {
        if (isWhitespace(c) || c === CharCodes.Gt) {
            ...
            ((__DEV__ || !false) && c === CharCodes.DoubleQuote) || c === CharCodes.SingleQuote || c === CharCodes.Lt || c === CharCodes.Eq || c === CharCodes.GraveAccent
        ) {
    ...
    private cleanup() {
        ...
        if (this.sectionStart !== this.index) {
            if (
                ...
                this.state === State.InAttrValueDq || this.state === State.InAttrValueSq || this.state === State.InAttrValueNq
            ) {
                this.cbs.onattribdata(this.sectionStart, this.index)
                this.sectionStart = this.index
            }
    ...
    private handleTrailingData() {
        ...
        if (this.state === State.InCommentLike) {
            ...
            this.state === State.InTagName || this.state === State.BeforeAttrName || this.state === State.BeforeAttrValue || this.state === State.AfterAttrName || this.state === State.InAttrName || this.state === State.InDirName || this.state === State.InDirArg || this.state === State.InDirDynamicArg || this.state === State.InDirModifier || this.state === State.InAttrValueSq || this.state === State.InAttrValueDq || this.state === State.InAttrValueNq || this.state === State.InClosingTagName
        ) {
