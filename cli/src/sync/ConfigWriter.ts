...
export interface FileChange {readonly before: FileSnapshot, readonly after: string}
...
export class ConfigWriter {
    async write(rootDir: string, changes: readonly FileChange[], guards: readonly FileSnapshot[] = []): Promise<void> {
        ...
        try { await writeFile(join(rootDir, ".arrange", ".gitignore"), "*\n", {flag: "wx"}) } catch (error) {
    ...
    protected async replaceFile(before: FileSnapshot, after: string, id: string): Promise<void> {
        ...
        try {
            ...
            if (before.content === null) {
                // 原子发布已写好的新文件；同名目标出现时 link 失败，不覆盖。
                await link(temporary, before.path)
