...
标准 Arrange 工程根目录不是 Node 项目，而是由 `arrange.project.yaml`、`arrange.local.yaml`、`ui/`、`native/` 与 `artifacts/` 组成。若把 CLI 作为 `@arrange/framework` 的项目依赖，会迫使根目录承担 Node 项目职责，破坏工程边界。
