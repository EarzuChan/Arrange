...
DynamicArrangable 明确接收 is 定义、props 参数对象与内容。三者作为完整调用输入校验，目标变化不能带旧参数初始化新实例；定义与重排 key 不变则复用实例。具名内容按最终选中定义校验，不产生槽位实例，也不透传未声明的顶层参数。

KeepAlive 以 cacheKey 保存默认内容的一次调用作用域，允许零项或多项内容。缓存默认容量 10，max 必须为正整数，按最近使用顺序逐出。重排 key 仍标识 KeepAlive 自身，不能代替 cacheKey。

停用保留实例、上下文与逻辑状态，暂停结构、值、观察与动画，退出视觉和交互资格；恢复按最新输入重排。原生节点、Modifier 和回调可在停用时退休，恢复重新物化，不承诺 ID 或 handle 永生。输入文本由逻辑实例保留，滚动由 ScrollState 保留；焦点、捕获及 IME 不自动恢复抢占。

激活和停用成功通知随相应提交发出，供业务暂停外部订阅。外部异步工作不会因暂停 effect 自动停止；结果可更新尚存活的逻辑状态，但不能复活停用节点，销毁后的迟到结果必须失效。逐出及所有者卸载执行最终清理，时序见 [调度线程与帧阶段](26-调度线程与帧阶段.md)。
