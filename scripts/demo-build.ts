...
export async function buildDemoNative(buildType: string | undefined): Promise<void> {
    ...
    await runInVsDev(`"${cmake}" -S . -B ${buildDir} -G Ninja -DCMAKE_MAKE_PROGRAM="${ninja}" -DCMAKE_BUILD_TYPE=${normalizedBuildType} -DARRANGE_WITH_QUICKJS_NG=ON -DARRANGE_BUILD_DEMO=ON -DARRANGE_BUILD_TESTS=ON${toolchainArgs()}${juceDirArg}${quickJsDirArg}`)
