...
namespace {
    ...
    void verifyBindingsAndFailureBoundary() {
        ...
        check(scene.bindingCount() == 1 && frame.content.drawOps.front().color == 0xff112233, "slot did not reach paint");
        ...
        check(!moved.error && moved.plan.layout && !moved.plan.measure && frame.content.drawOps.front().rect.x == 30, "typed placement failed");
        ...
        const auto previousColor = frame.content.drawOps.front().color;
        ...
        check(frame.revision == previousRevision && frame.content.drawOps.front().color == previousColor, "failed frame changed published content");
