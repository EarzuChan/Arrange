...
namespace arrange::core {
    ...
    inline char32_t decodeUtf8Codepoint(std::string_view text, std::size_t& index) noexcept {
        ...
        if ((current & 0xe0u) == 0xc0u && hasContinuation(1)) {
            const auto codepoint = ((current & 0x1fu) << 6u) | (static_cast<unsigned char>(text[index + 1]) & 0x3fu);
            index += 2;
            return static_cast<char32_t>(codepoint);
        }

        if ((current & 0xf0u) == 0xe0u && hasContinuation(1) && hasContinuation(2)) {
            const auto codepoint = ((current & 0x0fu) << 12u) | ((static_cast<unsigned char>(text[index + 1]) & 0x3fu) << 6u) | (static_cast<unsigned char>(text[index + 2]) & 0x3fu);
            index += 3;
            return static_cast<char32_t>(codepoint);
        }

        if ((current & 0xf8u) == 0xf0u && hasContinuation(1) && hasContinuation(2) && hasContinuation(3)) {
            const auto codepoint = ((current & 0x07u) << 18u) | ((static_cast<unsigned char>(text[index + 1]) & 0x3fu) << 12u) | ((static_cast<unsigned char>(text[index + 2]) & 0x3fu) << 6u) | (static_cast<unsigned char>(text[index + 3]) & 0x3fu);
            index += 4;
            return static_cast<char32_t>(codepoint);
        }
