...
namespace arrange::juce {
    ...
    std::size_t TextInputLayoutModel::textIndexAtPoint(const arrange::core::ModifierInstance& instance, const std::string& text, float viewportX, float x, float y) const {
        const auto layout = this->layout(instance, text, viewportX);
        return textLayoutService_.byteIndexAtPoint(*layout.text, {x - layout.metrics.textLeft + layout.metrics.viewportX, y - layout.metrics.textTop});
    }
